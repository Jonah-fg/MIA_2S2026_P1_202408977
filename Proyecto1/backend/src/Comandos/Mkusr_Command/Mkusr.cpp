...
namespace Comandos{
    ...
    CommandResult Mkusr_Command(const vector<string>& tokens) {
        ...
        if (!Global::sesionActual.esRoot) {
            return {false, "ERROR: Solo root puede crear usuarios"};
        }

        //Parseo parámetros: -user, -pass, -grp
        string usuario, contrasena, grupo;
        string atributos = joinTokens(tokens);
        static const regex lexic(R"(-user=[^\s]+|-pass=[^\s]+|-grp=[^\s]+)", regex::icase);
        ...
        if (found.size() != tokens.size()) {
            for (const auto& token : tokens) {
                if (!regex_search(token, lexic)) {
                    return {false, "ERROR: Parámetro no reconocido: " + token + " en MKUSR"};
                }
            }
        }
        ...
        for (const auto& fun : found) {
            ...
            if (key=="-user") {
                ...
                if (value.empty()){
                    return {false, "ERROR: grupo vacío"};
                }
                grupo = value;
            } 
            else{
        ...
        if (grupo.empty()) {
            return {false, "ERROR: falta -grp"};
        }

        //Obtener partición, superbloque y leer users.txt
        ...
        if (!UsersUtils::ParsearUsersTXT(contenido, grupos, usuarios, errMsg)) {
            return {false, "ERROR: Error al parsea users.txt: " + errMsg};
        }

        //Verificacion que el grupo exista
        bool grupoExiste=false;
        for (const auto& g : grupos) {
            if (g.nombre == grupo && !g.eliminado){
                grupoExiste = true;
                break;
            }
        }
        if(!grupoExiste) {
            return {false, "ERROR: El grupo '" + grupo + "' no existe o está eliminado"};
        }

        //Verificacion que el usuario no exista
        for (const auto& u : usuarios){
            if (u.nombre == usuario && !u.eliminado) {
                return {false, "ERROR: El usuario '" + usuario + "' ya existe"};
            }
        }
        //nuevo usuario
        int nuevoUID = UsersUtils::SiguienteUID(usuarios);
        UsersUtils::Usuario nuevoUsuario;
        nuevoUsuario.uid= nuevoUID;
        ...
        return {true, "MKUSR: Usuario '" + usuario + "' creado con UID " + to_string(nuevoUID)};
    }
